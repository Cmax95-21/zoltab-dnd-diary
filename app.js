...
class CampaignManager {
    ...
    handleSearch(query) {
        ...
        // FIX: Aggiunto .timeline-session al selettore per includere le sessioni nella ricerca
        document.querySelectorAll('.card, .timeline-day, .timeline-session').forEach(element => {
    ...
    clearSearch() {
        // FIX: Aggiunto .timeline-session anche al clear per consistency
        document.querySelectorAll('.card, .timeline-day, .timeline-session').forEach(element => {
            element.style.display = '';
            element.classList.remove('search-highlight');
        });
        ...
        try {
            ...
            for (const checkbox of checkedEntities) {
                ...
                if (entityType === 'characters') {
                    ...
                    entityData.type = '';
                    timelineData.organizations.push(entityName);
    ...
    renderTimeline() {
        ...
        sortedSessions.forEach(session => {
            ...
            // FIX PRINCIPALE: Aggiungi bottoni Modifica/Elimina alle card timeline
            sessionElement.innerHTML = `
                ...
                <div class="card-actions master-only" style="margin-top: 1rem;">
                    <button class="btn btn--small btn--secondary" onclick="event.stopPropagation(); campaignManager.openEntityModal('timeline', '${session.id}')">Modifica</button>
                    <button class="btn btn--small btn--danger" onclick="event.stopPropagation(); campaignManager.deleteEntity('timeline', '${session.id}')">Elimina</button>
                </div>
            `;

            // Event listener per la sessione stessa (per aprire il dettaglio)
            sessionElement.addEventListener('click', (e) => {
                // Se il click è su un tag o bottone, non aprire il dettaglio della sessione
                if (e.target.classList.contains('session-tag') || 
                    e.target.classList.contains('btn') ||
                    e.target.closest('.card-actions')) {
                    return; // Il tag/bottone gestisce il proprio click
                }
            ...
            // Usa la funzione per gestire i tag
            this.attachSessionTagListeners(sessionElement);
